      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className="glass card-elevated rounded-2xl p-6">
          ...
          <label className="mt-4 flex items-center justify-between">
            ...
            <input
              type="checkbox"
              className="h-5 w-9 cursor-pointer appearance-none rounded-full bg-white/10 checked:bg-cyan transition"
        ...
        <div className="glass card-elevated rounded-2xl p-6">
          ...
          <div className="mt-4 space-y-2">
            ...
            {socials.map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2 text-sm">
                <span className="w-20 text-xs uppercase tracking-widest text-muted-foreground">{s.platform}</span>
                <a href={s.url} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate hover:text-cyan">{s.url}</a>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={addSocial}
              className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-4 py-2 text-sm hover:bg-white/15">
...
function Field({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <label className="block">
      ...
      <input {...props}
        className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm outline-none placeholder:text-muted-foreground/60 focus:border-cyan/50 disabled:opacity-60" />
    </label>
  );
}
